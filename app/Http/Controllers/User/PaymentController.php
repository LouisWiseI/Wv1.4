...
class PaymentController extends Controller
{
    ...
    public function approvedCinetPay(Request $request)
    {
        if ($request->has('transaction_id') && session()->get('cinet_pay_id') == $request->get('transaction_id')) {
            try {
                $platform = PaymentPlatform::whereName("CinetPay")->first();
                $paymentPlatform = $this->paymentPlatformResolver->resolveService($platform->id);
                return $paymentPlatform->handleApproval($request);
            } catch (Exception $e) {
                toastr()->error(__('There was an error while retrieving payment gateway. Please try again'));
                return redirect()->back();
    ...
    public function approvedPrepaid(Request $request)
    {
        if (session()->has('paymentPlatformID')) {
            $paymentPlatform = $this->paymentPlatformResolver->resolveService(session()->get('paymentPlatformID'));
            return $paymentPlatform->handleApproval($request);
     ...
     */
    public function generatePaymentInvoice(Request $request, $order_id)
    {
        $this->generateInvoice($request, $order_id);
    }
     ...
     */
    public function showPaymentInvoice(Request $request, Payment $id)
    {
        if ($id->gateway == 'BankTransfer' && $id->status != 'completed') {
            $this->bankTransferInvoice($request, $id->order_id);
        } else {
            $this->showInvoice($request, $id);
        }
    }
