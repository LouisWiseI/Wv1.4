...
trait InvoiceGeneratorTrait
{
     ...
     *
     */
    public function generateInvoice($request, $order_id)
    {
        ...
        $convertedAmount = ceil(Http::get("https://api.exchangerate.host/convert?from=" . strtoupper($plan->currency) . "&to=" . strtoupper($devise) . "&amount=" . $total)->json('result'));
        if ($convertedAmount % 5 != 0 && $devise == 'XAF') {
            $convertedAmount = $convertedAmount + (5 - ($convertedAmount % 5));
        }
        $final_total = number_format($total, 2);
        $final_total .= " (" . $convertedAmount . " " . $devise . ")";
    ...
    public function bankTransferInvoice($request, $order_id)
    {
        ...
        $invoice->addTotal("Total", $total);
        $invoice->addTotal("VAT " . config('payment.payment_tax') . "%", $tax_total);
        $invoice->addTotal("Total Due", $final_total, true);
    ...
    public function showInvoice(Request $request, Payment $id)
    {
        $invoice_rows = ['invoice_currency', 'invoice_language', 'invoice_vendor', 'invoice_vendor_website', 'invoice_address', 'invoice_city', 'invoice_state', 'invoice_postal_code', 'invoice_country', 'invoice_phone', 'invoice_vat_number'];
        $invoice = [];
        $settings = Setting::all();
